...
## 安装（推荐 ToolPkg）
1. 下载插件包 `root_guard-v1.0.0.toolpkg`（见 [Releases](https://github.com/shengbuding/operit-root-guard/releases/latest)，或本仓库 `dist/` 目录）
2. 把 `.toolpkg` 放入设备目录：
   `Android/data/com.ai.assistance.operit/files/packages/`
3. 打开 Operit → 工具箱/包管理 → 导入并启用「Root 安全护栏」
4. 对 AI 说：
   > 调用 root_guard 的 guard_setup 和 guard_status

> 旧版单文件格式仍可用：复制本仓库 `packages/root_guard.js` 到同一目录 → 启用即可。
...
## 配合 KSU 模板（推荐）
仓库 `docs/operit.daily.json`：KSU 最小权能模板（无 SYS_ADMIN/SYS_BOOT）。
...
Install (recommended, ToolPkg): download `root_guard-v1.0.0.toolpkg` from [Releases](https://github.com/shengbuding/operit-root-guard/releases/latest) (or repo `dist/`), put it into `Android/data/com.ai.assistance.operit/files/packages/`, import & enable in Operit, then run guard_setup.

Single-file alternative: copy `packages/root_guard.js` to the same folder.
